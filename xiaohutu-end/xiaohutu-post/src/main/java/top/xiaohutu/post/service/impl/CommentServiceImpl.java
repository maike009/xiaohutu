...
public class CommentServiceImpl implements ICommentService {
    ...
    public List<CommentVO> selectCommentList(Comment comment) {
        ...
        List<CommentVO> commentVOList = commentMapper.selectCommentList(comment);
        // 获取顶级评论
        List<CommentVO> topComments = commentVOList.stream()
                .filter(commentVo -> commentVo.getTopId() == 0)
                .collect(Collectors.toList());
        // 获取顶级评论下的评论
        for (CommentVO topComment : topComments) {
            List<CommentVO> ComChildren = commentVOList.stream().filter(commentVo ->
                    commentVo.getTopId().equals(topComment.getCommentId())
            ).collect(Collectors.toList());
            topComment.setChildren(ComChildren);
        }
        return topComments;
    }
